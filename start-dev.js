...
// On Render / Production: Run production server directly
if (isProduction) {
  const port = process.env.PORT || 3000;
  const isBackend = process.env.SERVICE_TYPE === 'backend' || process.env.IS_BACKEND === 'true';

  if (isBackend) {
    console.log(`🚀 Starting Backend in Production mode on port ${port}...`);
    require('./backend/server.js');
  } else {
    console.log(`🚀 Starting Frontend Next.js in Production mode on port ${port}...`);
    const frontendProcess = spawn(`npx next start -H 0.0.0.0 -p ${port}`, {
