...
const DEFAULT_PROPS = {
  ...
  UnrealBloomPassStrength: 1.4,
  UnrealBloomPassRadius: 0.5,
  UnrealBloomPassThreshold: 0.15,

  BeamColorCore: '#38bdf8',
  BeamColorGlow: '#2563eb',
  BeamWidth: 0.35,
  BeamSoftness: 1.8,
  BeamNoiseScale: 3.0,
  BeamNoiseAmp: 0.25,
  ...
  PoolInnerColor: '#38bdf8',
  PoolOuterColor: 'rgba(3, 105, 161, 0)',
  PoolSize: 7,
  PoolYPosition: -1.2,
  PoolOpacity: 0.8,
  ...
  ParticleInnerColor: '#60a5fa',
  ParticleOuterColor: 'rgba(37, 99, 235, 0)',

  PoolScaleFrequency: 1.5,
  PoolOpacityFrequency: 1.2,
  ParticleRotationSpeed: 0.08,
  BeamIntensityFrequency: 1.8,
};

export default function ShiningStar(userProps = {}) {
  ...
  useEffect(() => {
    ...
    try {
      renderer = new THREE.WebGLRenderer({
        ...
        powerPreference: p.RendererPowerPreference,
        alpha: true,
      });
    } catch {
      queueMicrotask(() => {
    ...
    renderer.setPixelRatio(Math.min(window.devicePixelRatio || 1, p.RendererSetPixelRatio));
    ...
    if (debugInfo) {
      ...
      if (isLowEnd) {
        ...
        localParticleCount = Math.max(500, Math.floor(localParticleCount * 0.45));
      }
    }

    // Scene & Camera
    const scene = new THREE.Scene();
    if (p.SceneBackgroundColor) {
    ...
    const makeCircleTexture = (size, inner, outer) => {
